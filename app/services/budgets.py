...
from .common import ValidationError, money, owned_or_404, user_account_ids
...
def delete_budget(user, budget_id):
    budget = owned_or_404(db.session.scalar(select(Budget).where(Budget.id == budget_id, Budget.user_id == user.id)))
...
def safe_to_spend(user_id, month=None):
    ...
    end = date(target.year + (target.month == 12), 1 if target.month == 12 else target.month + 1, 1)
    spendable_accounts = Account.ledger.has(user_id=user_id) & Account.include_in_safe_to_spend.is_(True)
    balance = money(db.session.scalar(select(func.coalesce(func.sum(Account.opening_balance), 0)).where(spendable_accounts)))
    directions = db.session.execute(
        select(Transaction.direction, func.coalesce(func.sum(Transaction.amount), 0))
        .where(Transaction.posted_at < end, Transaction.account.has(spendable_accounts))
        .group_by(Transaction.direction)
    ).all()
    totals = {direction: money(total) for direction, total in directions}
