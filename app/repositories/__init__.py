...
class TransactionRepository:
    ...
    @staticmethod
    def search(user_id, *, date_from=None, date_to_exclusive=None, account_id=None, category_id=None, direction=None, page=1, per_page=20):
        ...
        if date_to_exclusive:
            # The public filter is an inclusive calendar date, while posted_at
            # is a DATETIME. Comparing with midnight dropped imported rows from
            # the final day; a half-open interval preserves every timestamp.
            query = query.where(Transaction.posted_at < date_to_exclusive)
        if account_id:
