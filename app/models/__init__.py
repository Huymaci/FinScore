...
from sqlalchemy.dialects.mysql import BINARY, DATETIME, LONGTEXT
...
class User(UserMixin, db.Model):
    ...
    __table_args__ = (CheckConstraint("role IN ('USER','ADMIN','SUPPORT_ADMIN')", name="ck_users_role"),)
...
class Account(db.Model):
    ...
    include_in_safe_to_spend: Mapped[bool] = mapped_column(default=True, nullable=False)
...
class ImportBatch(db.Model):
    ...
    # MySQL TEXT holds 65,535 *bytes*. A 183-row statement with Vietnamese
    # descriptions, a per-row dedup_key hex and a category reason sentence
    # overruns that and fails with error 1406; SQLite has no such limit, so the
    # unit tests could not see it. LONGTEXT keeps headroom for the largest
    # upload MAX_CONTENT_LENGTH permits.
    preview_json: Mapped[str] = mapped_column(Text().with_variant(LONGTEXT(), "mysql"), nullable=False)
    created_at: Mapped[datetime] = mapped_column(UTC_DATETIME, default=utcnow, nullable=False)
    reverted_at: Mapped[datetime | None] = mapped_column(UTC_DATETIME)
...
class HabitChallenge(db.Model):
    __tablename__ = "habit_challenges"
    id: Mapped[int] = mapped_column(primary_key=True)
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    category_id: Mapped[int] = mapped_column(ForeignKey("categories.id", ondelete="CASCADE"), nullable=False)
    habit_key: Mapped[str] = mapped_column(String(160), nullable=False)
    habit_name: Mapped[str] = mapped_column(String(160), nullable=False)
    ...
    saved_amount: Mapped[int] = mapped_column(BigInteger, default=0, nullable=False)
    status: Mapped[str] = mapped_column(String(20), default="PROPOSED", nullable=False)
    created_at: Mapped[datetime] = mapped_column(UTC_DATETIME, default=utcnow, nullable=False)
    category: Mapped[Category] = relationship()
    __table_args__ = (
        CheckConstraint("baseline_count > 0", name="ck_habit_challenges_baseline"),
        CheckConstraint("target_count > 0", name="ck_habit_challenges_target"),
        CheckConstraint("average_amount > 0", name="ck_habit_challenges_average"),
        CheckConstraint("status IN ('PROPOSED','ACTIVE','COMPLETED','FAILED','DECLINED')", name="ck_habit_challenges_status"),
        Index("ix_habit_challenges_user_status", "user_id", "status"),
    )
...
class AuditLog(db.Model):
    ...
    # FR-05 stores `ACCOUNT_DELETED:<salt hex>:<sha256 hex>` here, which is 113
    # characters; 100 overflowed under MySQL strict mode. See migration 08.
...
class SupportReport(db.Model):
    ...
    __table_args__ = (
        CheckConstraint("status IN ('OPEN','RESOLVED')", name="ck_support_reports_status"),
        Index("ix_support_reports_status_created", "status", "created_at"),
    )


__all__ = ["User", "Ledger", "Account", "Category", "Transaction", "ImportTemplate", "ImportBatch", "ImportError", "CategorizationRule", "Budget", "Alert", "HabitChallenge", "AuditLog", "SupportReport"]
