...
@login_required
def list_accounts():
    accounts = list(AccountRepository.list_for(current_user.id))
    balances = service.current_balances(accounts)
    return jsonify(items=[account_json(item, balances[item.id]) for item in accounts])
...
@accounts_bp.delete("/<int:account_id>/permanent")
@login_required
def permanently_delete_account(account_id):
    return validation_response(
        lambda: (
            service.permanently_delete(current_user, account_id),
            jsonify(message="Đã xóa vĩnh viễn tài khoản"),
        )[1]
    )
